...
- **单工具执行超时**：`tool_timeout_seconds`（默认 90s）作为【默认】值；超时立即终止并返回失败，交由 AI 接管。AI 调用工具时可传 `timeout_seconds` 覆盖单次等待时长（如下载大文件传更大值），默认值仅在 AI 未指定时生效；联网 / 脚手架 / execute 自终止，ask_user / task / request_access 豁免。
...
| 超时保护 | 单工具 / 单模型请求分别有硬超时，超时交由 AI 接管，可随时终止；工具超时是默认值，AI 可传 `timeout_seconds` 单次覆盖 |
