...
from autobee.core.skills_store import SkillsStore, SkillInfo
...
class SkillsWorkspace(QWidget):
    ...
    def _build(self):
        ...
        hl.addWidget(title)
        tip = QLabel(
            "技能以文件夹 + SKILL.md 形式存放在全局目录（默认 ~/.wokbee/autobee/skills）。"
            "启用后，AutoBee 运行时通过 /skills/ 只读挂载该目录，不会复制到每个项目。"
            "供 Deep Agents 按需调用。"
        )
