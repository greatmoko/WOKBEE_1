...
from wokbee.core.provider_store import ProviderStore
...
class AutoBeeSettingsWorkspace(QWidget):
    """AutoBee 必要配置。"""
    ...
    def _build(self):
        ...
        self._ws_edit.setPlaceholderText("选择 AutoBee 工作区根目录…")
        ...
        hint = QLabel(
            ...
            "经验→memory/EXPERIENCE.md；Skills 全局挂载不复制"
        ...
        model_hint = QLabel(
            "选项来自「厂商设置」中已启用的模型；Key / Host 仍在厂商设置中维护。"
        )
        ...
        btn_bar.addStretch()
        save_btn = QPushButton("保存 AutoBee 设置")
    ...
    def _browse_workspace(self):
        ...
        path = QFileDialog.getExistingDirectory(self, "选择 AutoBee 工作区根目录", start)
        if path:
    ...
    def _on_save(self):
        ...
        self.settings.save()
        _tip(self, self.theme, "AutoBee 设置已保存。")
