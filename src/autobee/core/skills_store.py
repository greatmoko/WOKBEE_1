"""全局 Skills 存储（~/.wokbee/autobee/skills/）。"""
...
DEFAULT_SKILLS_ROOT = Path.home() / ".wokbee" / "autobee" / "skills"
...
class SkillsStore:
    ...
    @property
    def root(self) -> Path:
        raw = self._config.get("autobee.skills_root") or ""
        if str(raw).strip():
            return Path(str(raw)).expanduser()
        return DEFAULT_SKILLS_ROOT

    def set_root(self, path: str | Path) -> None:
        self._config.set("autobee.skills_root", str(path))
        self._config.save()
        Path(path).mkdir(parents=True, exist_ok=True)

    def _enabled_map(self) -> dict[str, bool]:
        raw = self._config.get("autobee.skills_enabled")
        if isinstance(raw, dict):
            return {str(k): bool(v) for k, v in raw.items()}
        return {}

    def _save_enabled_map(self, data: dict[str, bool]) -> None:
        self._config.set("autobee.skills_enabled", data)
        self._config.save()
...
""",
    ...
    def cleanup_project_copies(self, project_root: Path) -> None:
        """清理历史上同步进项目的 /skills 副本（带 .autobee_synced 标记的）。"""
        dest_root = Path(project_root) / "skills"
        marker = dest_root / ".autobee_synced"
