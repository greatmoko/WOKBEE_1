...
def default_data_dir() -> Path:
    """应用数据目录：优先 ~/.wokbee；旧 ~/.tokbee 自动迁移（rename 失败则复制）。"""
    preferred = Path.home() / ".wokbee"
    legacy = Path.home() / ".tokbee"
    if preferred.exists():
        return preferred
    if legacy.exists():
        try:
            ...
            # Windows 上目录被占用时 rename 常失败，改为复制到新目录
            try:
                shutil.copytree(legacy, preferred, dirs_exist_ok=True)
                return preferred
            except OSError:
                return legacy
...
class Config:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        if config_path:
            ...
            self._path = default_data_dir() / "config.json"
