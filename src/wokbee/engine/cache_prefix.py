...
def static_system_prompt(*, mode: str) -> str:
    ...
    if mode == "chat":
        return (
            ...
            "文件工具路径（read_file/write_file/edit_file/ls/grep/glob）**一律用虚拟路径**："
            "项目内用 workspace/a.json、deliverables/out.md、uploads/f.pdf；"
            "项目外先 request_access(path, reason) 申请**人工高危审批**，获批后用 /ext/<slug>/… 访问。"
            "**禁止** 把真实主机路径（C:\\\\...、D:/...）传给文件工具；只有 execute 接受真实路径。\n"
            "工作区：workspace/；交付物：deliverables/；上传：uploads/；记忆：memory/；"
    ...
    return (
        ...
        "文件工具路径（read_file/write_file/edit_file/ls/grep/glob）**一律用虚拟路径**："
        "项目内用相对项目根的 workspace/a.json、deliverables/out.md、uploads/f.pdf；"
        "项目外先调用 request_access(path, reason) 申请**人工高危审批**，获批后用返回的 "
        "/ext/<slug>/… 虚拟路径访问。**禁止**把真实主机路径（C:\\\\...、D:/...、"
        "C:/...）传给文件工具，否则报错——只有 execute 才可用本机真实路径。\n"
        "已授权附加目录会列出虚拟路径（/ext/<slug>/… 对应用户批准的目录，见【会话上下文】）。"
        "项目根为工作根；工作区：workspace/；交付物：deliverables/；上传：uploads/；"
        "记忆：memory/；参考材料：references/。\n"
        "当你使用外部软件/服务、需登录、或依赖环境参数/密钥时，请把可复用的第三方代码、"
        "配置、环境参数与登录信息保存到 references/，并在 references/MANIFEST.md 登记，"
        "确保下次能稳定复跑；references/ 不会被归档。这些敏感信息仅供本机使用，勿外发。\n"
