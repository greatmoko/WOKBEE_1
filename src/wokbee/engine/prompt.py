...
def static_system_prompt(*, mode: str) -> str:
    ...
    if mode == "chat":
        return (
            ...
            "list_credentials / get_credential（登录凭据：只给环境变量名，禁止在回复或命令里写出账号密码）。\n"
            "联网：需要高质量、最新、多轮检索且带引用的资料时优先 deepseek_web_search；"
            "普通快捷查询用 web_search/http_get 即可。\n"
            "下载大文件/长耗时任务可在调用工具时传 `timeout_seconds`（秒）覆盖默认超时；"
            "未传则用默认值。\n"
    ...
    return (
        ...
        "登录其它系统：用 list_credentials / get_credential；工具只返回环境变量名，"
        "execute 时密码已注入进程环境。严禁在回复、execute 命令或文件中写出账号密码。\n"
